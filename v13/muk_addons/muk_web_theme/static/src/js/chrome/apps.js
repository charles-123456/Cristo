...
AppsMenu.include(_.extend({}, MenuSearchMixin, {
    events: _.extend({}, AppsMenu.prototype.events, {
        ...
        "click .full": "_hideNavBarMenus",
        "click .dropdown-menu": "_showNavBarMenus",
    }),
...
//            "background-image": "url(" + url + ")"
    ...
    _hideNavBarMenus: function(event) {
    	if ($(event.target)[0].getAttribute('aria-expanded') == "true"){
    		$('.o_menu_brand, .o_menu_sections').show();
    	}else{
    		$('.o_menu_brand, .o_menu_sections').hide();
    	}
    },
    _showNavBarMenus: function(event) {
  		$('.o_menu_brand, .o_menu_sections').show();
    },
}));
