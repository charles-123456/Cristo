...
#         'muk_dms.mixins.thumbnail',
    ...
    # Database
    #----------------------------------------------------------
    thumbnail = fields.Char(compute="_compute_extension",string="Thumbnail Path",store=False,readonly=True)
    img = fields.Image("Image",readonly=True,store=False)
    name = fields.Char(
        string="Filename", 
        required=True,
        index=True)
    ...
    def _get_thumbnail_placeholder_name(self):
        for rec in self:
            return rec.extension and "file_%s.svg" % rec.extension or ""
    ...
    def _compute_extension(self):
        ...
        for record in self:
            ...
            ph_name = "file_%s.svg" %(record.extension)
            path = "/muk_dms/static/src/img/thumbnails/%s" %(ph_name)
            image_path = get_module_resource('muk_dms', 'static/src/img/thumbnails', '%s'%(ph_name))
            if not image_path:
                image_path = get_module_resource('muk_dms', 'static/src/img/thumbnails', 'file_unkown.svg')
                path = "/muk_dms/static/src/img/thumbnails/file_unkown.svg"
            if image_path:
                record.img = base64.b64encode(open(image_path, 'rb').read())
